  ...
  switch (state) {
    case STRAIGHT:  // straight
      switch (Maze::checkWalls(
          true)) {  // base straight movement off walls in next square
        case 0:     // no walls
      ...
      moveSpeedRight =
          (moveSpeedRight - SLOWEST) / SQUARE_SIZE *
              (SQUARE_SIZE - SensorController::rightEncoder.read()) +
          SLOWEST;
      moveSpeedLeft = (moveSpeedLeft - SLOWEST) / SQUARE_SIZE *
                          (SQUARE_SIZE - SensorController::leftEncoder.read()) +
                      SLOWEST;
    ...
    case REVERSED:
      ...
      // ramp down speed linearly
      moveSpeedRight =
          (moveSpeedRight - SLOWEST) / SQUARE_SIZE_REVERSED *
              (SQUARE_SIZE_REVERSED - SensorController::rightEncoder.read()) +
          SLOWEST;
      moveSpeedLeft =
          (moveSpeedLeft - SLOWEST) / SQUARE_SIZE_REVERSED *
              (SQUARE_SIZE_REVERSED - SensorController::leftEncoder.read()) +
          SLOWEST;
    ...
    case TURN_RIGHT:
      // turnSum = abs(SensorController::rightEncoder.read()) +
      // abs(SensorController::leftEncoder.read()); moveSpeedRight =
      // (BASE_POWER+20 - SLOWEST)/TURN_ENCODER_THRESH *
      // (TURN_ENCODER_THRESH-turnSum) + SLOWEST; moveSpeedLeft =
      // (BASE_POWER+20- SLOWEST)/TURN_ENCODER_THRESH *
      // (TURN_ENCODER_THRESH-turnSum) + SLOWEST; if( -
      // SensorController::rightEncoder.read() >
      // SensorController::leftEncoder.read()){
      //   moveSpeedRight = TURN_POWER - 5;
      //   moveSpeedLeft  = TURN_POWER + 5;
      // } else if(-SensorController::rightEncoder.read()<
      // SensorController::leftEncoder.read()){
    ...
    case TURN_LEFT:
      ...
      // abs(SensorController::leftEncoder.read()); moveSpeedRight =
      // (BASE_POWER+20- SLOWEST)/TURN_ENCODER_THRESH *
      // (TURN_ENCODER_THRESH-turnSum) + SLOWEST; moveSpeedLeft = (BASE_POWER+20
      // - SLOWEST)/TURN_ENCODER_THRESH * (TURN_ENCODER_THRESH-turnSum) +
      // SLOWEST;
      // if( -SensorController::leftEncoder.read() >
      // SensorController::rightEncoder.read()){
      //   moveSpeedRight = TURN_POWER + 5;
      //   moveSpeedLeft  = TURN_POWER - 5;
      // } else if( -SensorController::leftEncoder.read() <
      // SensorController::rightEncoder.read()){
      //   moveSpeedRight = TURN_RIGHT - 5;
      //   moveSpeedLeft  = TURN_POWER + 5;
      // } else {
...
void MovementController::brake(int state) {
  if (MovementController::left->state != 0 &&
      MovementController::right->state != 0) {
